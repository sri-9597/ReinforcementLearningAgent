...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        qStates = self.mdp.getTransitionStatesAndProbs(state, action)
        qSum = 0
        for nextState, probability in qStates:
            qSum += probability * (
...
class AsynchronousValueIterationAgent(ValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        statesLen = len(allStates)
        for k in range(self.iterations):
            currState = allStates[k % statesLen]
            if self.mdp.isTerminal(currState):
                continue
            vStar = -float("inf")
            for action in self.mdp.getPossibleActions(currState):
                v = self.computeQValueFromValues(currState,action)
                if v > vStar:
                    vStar = v
            self.values[currState] = vStar
